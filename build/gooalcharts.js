(function (global, factory) {
  ...
  function drawPie(dom, data, opt, newWidth) {
      ...
      var color = ordinal().range(['#0c6ebb', '#11bce8', '#9beffa', "#6b486b", "#a05d56", "#d0743c", "#ff8c00"]);
  ...
  function drawLine(dom, data, opt, newWidth) {
      ...
      // 比例尺
      yScale$7 = linear$2().domain([min(ys), max(ys)]).range([height$14 - margin.bottom - margin.top, 0]).nice();

      //隐形坐标轴测坐标宽度
      var hideYAxis = lineSVG.append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")").style("opacity", 0).call(axisLeft().scale(yScale$7));
      var yAxisBBox = hideYAxis.node().getBBox();
      margin.left = yAxisBBox.width + margin.left;
      ...
      xScale$7 = linear$2().domain([min(xs), max(xs)]).range([0, width$14 - margin.left - margin.right]).nice();
