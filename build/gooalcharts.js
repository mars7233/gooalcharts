(function (global, factory) {
  ...
  function drawBar(dom, opt) {
      ...
      // 比例尺
      xScale = band().domain(sequence(1, dataset.length + 1)).range([1, width - margin.right - margin.left]).paddingInner(0.2).paddingOuter(0.1);

      yScale = linear$2().domain([0, max(dataset)]).rangeRound([height - margin.bottom - margin.top, 0]);
      ...
      // 绘制数据
      columnSVG.selectAll("rect").data(dataset).enter().append("rect").attr("class", "myrect").attr("x", function (d, i) {
          return margin.left + xScale(i + 1);
      }).attr("y", function (d, i) {
          return height - margin.bottom;
      }).attr("width", xScale.bandwidth).transition().attr("y", function (d, i) {
          return margin.top + yScale(d);
      }).attr("height", function (d) {
          return height - yScale(d) - margin.bottom - margin.top;
  ...
  var barContainer;
  var tooltip$1;
  var commonOpt$1, axisBox$1, dataBox$1;

  // 读取配置文件
  function readConfig$1(options) {
    commonOpt$1 = options;
    axisBox$1 = commonOpt$1.axisBox;
    dataBox$1 = commonOpt$1.dataBox;
  }
  ...
  function drawColumn(dom, options) {
    ...
    // 绘制容器
    barContainer = dom.append("svg").attr("width", width$1).attr("height", height$1).attr("class", "column");
