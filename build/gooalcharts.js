(function (global, factory) {
  ...
  function drawSimple(dom, options) {
    // 柱状图
    var svg_1 = select(dom).append("svg").attr("width", w).attr("height", h_2);
    console.log(dom);
    ...
    var dataset_2_x_max = max(dataset_2, function (d) { return d[0]; });
    var dataset_2_y_max = max(dataset_2, function (d) { return d[1]; });
    // 比例尺
    var xScale = linear$2().domain([0, dataset_2_x_max]).range([padding, w - padding]);
    var yScale = linear$2().domain([0, dataset_2_y_max]).range([padding, h_2 - padding]);
  }
