...
function drawAxis(chart, opt, newWidth) {
    ...
    // x轴
    if (xtitle != "") {
        svg.append("text").attr("class", opt.type + "xTitle" + opt.id).attr("transform", "translate(" + ((width$6 - margin.left - margin.right) / 2 + margin.left) + "," + (height$6 - margin.bottom + 15 + xAxisBBox.height) + ")").attr("text-anchor", "middle").text(xtitle);
    }
    // y轴
    if (ytitle != "") {
