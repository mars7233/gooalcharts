(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('fs'), require('http'), require('rollup/dist/typings/ast/nodes/NewExpression'), require('constants'), require('rollup/dist/typings/ast/nodes/TemplateElement')) :
  typeof define === 'function' && define.amd ? define(['exports', 'fs', 'http', 'rollup/dist/typings/ast/nodes/NewExpression', 'constants', 'rollup/dist/typings/ast/nodes/TemplateElement'], factory) :
  (factory((global.gooal = {}),global.fs,global.http,global.NewExpression,global.constants,global.TemplateElement));
}(this, (function (exports,fs,http,NewExpression,constants,TemplateElement) { 'use strict';
  ...
  var GooalCharts = function () {
      ...
      createClass(GooalCharts, [{
          ...
          value: function setLegendBox(legendOpt) {
              var legendBox = this.container.append("svg").attr("class", "legendBox").attr("id", this.getOptions().type + "LegendBox" + this.getId());
              if (legendOpt.show == true || this.getOptions().type == "bar") {
                  legendBox.attr("width", this.width * 0.2).attr("height", 400);
          ...
          value: function setDataBox(dataOpt) {
              var dataBox = this.container.append("svg").attr("class", "dataBox").attr("id", this.getOptions().type + "DataBox" + this.getId()).attr("width", this.getWidth() * 0.8).attr("height", 400);
              return dataBox;
          }
  ...
  function drawGroupedBar(dom, data, opt, newWidth) {
      ...
      if ("axisBox" in commonOpt$2) {
          var _axisBox = commonOpt$2.axisBox;
          if ("yAxis" in _axisBox) if ("title" in _axisBox.yAxis) {
              margin.left = margin.left + 20;
          }
          if ("xAxis" in _axisBox) {
      ...
      //隐形坐标轴测坐标宽度 
      var hideYAxis = columnSVG$2.append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")").style("opacity", 0).call(axisLeft().scale(yScale$2));
      var yAxisBBox = hideYAxis.node().getBBox();
      margin.left = yAxisBBox.width + margin.left;

      xScale_0 = band().domain(primaryItem).range([0, width$2 - margin.right - margin.left]).paddingInner(0.2).paddingOuter(0.1);

      xScale_1 = band().domain(secondaryItem).range([0, xScale_0.bandwidth()]).paddingInner(0.2);

      columnSVG$2.append("svg").selectAll("g").data(opt.data).enter().append("g").attr("transform", function (d) {
          return "translate(" + (margin.left + xScale_0(getObjFirstValue(d))) + "," + "0" + ")";
      }).selectAll("rect").data(function (d) {
          return secondaryItem.map(function (key) {
              return { category: key, key: getObjFirstValue(d), value: d[key] };
          });
      }).enter().append("rect").attr("class", commonOpt$2.type + "Element" + commonOpt$2.id).attr("x", function (d) {
          return xScale_1(d.category);
      }).attr("y", function (d, i) {
      ...
      return { "svg": columnSVG$2, "margin": margin, "xScale": xScale_0, "yScale": yScale$2 };
  }
  ...
  function drawGroupedBarHori(dom, data, opt, newWidth) {
      ...
      }).enter().append("rect").attr("class", commonOpt$3.type + "Element" + commonOpt$3.id).attr("y", function (d, i) {
          return yScale_1(d.category) - margin.bottom + margin.top;
      }).attr("x", function (d) {
          return margin.left;
  ...
  function drawGroupedBar2(dom, data, opt, newWidth) {
      ...
      columnSVG$4.selectAll("rect").data(opt.data).enter().append("rect").attr("class", commonOpt$4.type + "Element" + commonOpt$4.id).attr("x", function (d, i) {
  ...
  function handleBarData(opt) {
      ...
      data.forEach(function (element) {
          var key = getObjValue(0, element);
          var value = getObjValue(1, element);
  ...
  function mouseOverHighlight(d) {
      preColor = select(this).style("fill");
      // 悬浮高亮
  ...
  function presenter(dom, options, legendDom, newWidth) {
    ...
    if (options.type == "bar") {
      ...
      if (options.dataBox.direction == "horizontal") {
        ...
        drawLegend$1(legendDom, data$1.keyList, options);
      } else {
  ...
  var GooalTooltip = function () {
      function GooalTooltip(svg, opt, tooltipCon) {
          classCallCheck(this, GooalTooltip);
      ...
      createClass(GooalTooltip, [{
          key: "drawTooltip",
          value: function drawTooltip(svg, opt) {
              var commonOpt = opt;
              var chartEl = svg;
              // init
              ...
              chartEl.selectAll(elementClass).on("mouseover." + commonOpt.type + "tooptip" + commonOpt.id, this.tooltipConfig).on("mousemove." + commonOpt.type + "tooptip" + commonOpt.id, function (d) {
                  ...
                  tooltip.style("opacity", 0.0).style("left", "-100px").style("top", "-100px");
              });
          ...
          key: "redrawTooltips",
          value: function redrawTooltips(svg, opt, tooltipCon) {
              var commonOpt = opt;
              var chartEl = svg;
  ...
  var GooalBar = function (_GooalCharts) {
      ...
      createClass(GooalBar, [{
          ...
          value: function addTooltip(tooltipConfig) {
              ...
              var tooltip = new GooalTooltip(this.getBarSVG(), this.getOptions(), tooltipConfig);
  ...
  function handlePieData(opt) {
      ...
      pie$$1.forEach(function (element) {
          var key = getObjFirstValue(element.data);
          element.key = key;
          keys$$1.push(key);
      });
      pie$$1.keys = keys$$1;
  ...
  var preColor$2 = void 0;
  var preRadius = void 0;
  ...
  function scatter (dom, options, opt, newWidth) {
  ...
  var GooalScatter = function (_GooalCharts) {
      ...
      createClass(GooalScatter, [{
          ...
          key: 'addTooltip',
          value: function addTooltip(tooltipCon) {
              this.tooltipConfig = tooltipCon;
              var tooltip = new GooalTooltip(this.getScatterSVG(), this.getOptions(), tooltipCon);
              this.tooltip = tooltip;
              return tooltip.tooltip;
          }
      }, {
          ...
          key: 'addEvent',
          value: function addEvent(event, method) {
              return addEvents$2(this.getScatterSVG(), event, method, this.getOptions());
          }
      }, {
          key: 'draw',
          value: function draw() {
  ...
  function drawLine(dom, data, opt, newWidth) {
      ...
      var ys = [];
      data.forEach(function (element) {
          var y = getObjValue(1, element);
          ys.push(y);
      });

      // 比例尺
      yScale$8 = linear$2().domain([min(ys), max(ys)]).range([height$14 - margin.bottom - margin.top, 0]).nice();

      //隐形坐标轴测坐标宽度
      var hideYAxis = lineSVG.append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")").style("opacity", 0).call(axisLeft().scale(yScale$8));
      var yAxisBBox = hideYAxis.node().getBBox();
      margin.left = yAxisBBox.width + margin.left;

      var xs = [];
      data.forEach(function (element) {
          var x = getObjValue(0, element);
          xs.push(x);
      });

      xScale$8 = linear$2().domain([min(xs), max(xs)]).range([0, width$14 - margin.left - margin.right]).nice();

      // 线生成器

      var line_generator = line().x(function (d, i) {
          return xScale$8(d.x);
      }).y(function (d, i) {
          return yScale$8(d.y);
      });
      ...
      // 绘制数据
      lineSVG.append("path").attr("d", line_generator(data)).attr("fill", "none").attr("stroke", "steelblue").attr("stroke-width", "2px").attr("transform", "translate(" + margin.left + ", " + margin.top + ")");

      // 添加圆点
      lineSVG.selectAll("circle").data(data).enter().append("svg:circle").attr("cx", function (d) {
          return xScale$8(d.x);
      }).attr("cy", function (d) {
          return yScale$8(d.y);
      }).attr("transform", "translate(" + margin.left + ", " + margin.top + ")").attr("r", 5).attr("fill", "#1E90FF").on("mouseover", function () {
          select(this).attr("fill", "#000080").attr("stroke", "rgba(0, 0, 128, 0.5)").attr("stroke-width", "2px");
      }).on("mouseout", function () {
          select(this).transition().duration(250).attr("fill", "#1E90FF").attr("stroke", "none");
  ...
  var commonOpt$21 = void 0;
  var data$6 = void 0;

  function handleLineData(opt) {
      commonOpt$21 = opt;
      // 绑定数据
      data$6 = commonOpt$21.data;

      var dataSet = [];
      data$6.forEach(function (element) {
          var x = getObjValue(0, element);
          var y = getObjValue(1, element);
          var d = [];
          d.push(x);
  ...
  function presenter$3(dom, options, legendDom, newWidth) {
      ...
      if (options.type == "linechart") {
          ...
          drawAxis$1(lineChart, options, newWidth);
      }
