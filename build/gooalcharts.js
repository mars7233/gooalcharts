...
var GooalCharts = function () {
    ...
    createClass(GooalCharts, [{
        ...
        value: function titleBoxInit(titleOpt) {
            ...
            // 添加填充
            titleBox.append("rect").attr("width", "100%").attr("height", 40).style("fill-opacity", 0);
            return titleBox;
        }
    }, {
        key: "getTitleBox",
        ...
        value: function boxLayout() {
            ...
            this.titleBox.attr("y", function () {
                ...
                if (titleOpt.position == "bottom") {
                    titleBoxY = legendBBox.height + 400;
                }
                return titleBoxY;
            });
            ...
            this.dataBox.attr("y", function () {
                ...
                if (legendOpt.position == "top") {
                    dataBoxY = dataBoxY + legendBBox.height;
                }
                if (titleOpt.position == "bottom") {
                    dataBoxY = dataBoxY - titleBBox.height;
                }
                return dataBoxY;
            });
...
function drawColumn(dom, options) {
  ...
  // tooltip 初始化
  tooltip = select("body").append("div").attr("class", "tooltip").style("opacity", 0.0).style("position", "absolute").style("width", "120px").style("height", "auto").style("font-family", "simsun").style("font-size", "14px").style("text-align", "center").style("border-style", "solid").style("border-width", "1px").style("background-color", "white").style("border-radius", "5px");

  // 绘制容器
  var svg = dom.append("svg").attr("width", w).attr("height", h).attr("class", "column");

  // 比例尺
  xScale = band().domain(sequence(1, dataset_1.length + 1)).range([margin.left, w - margin.right]);
  yScale = linear$2().domain([0, max(dataset_1)]).rangeRound([h - margin.bottom - margin.top, 0]);

  // 绘制坐标轴
  var xAxis = axisBottom().scale(xScale);
  var yAxis = axisLeft().scale(yScale);
  svg.append("g").attr("transform", "translate(" + 0 + "," + (h - margin.bottom) + ")").attr("class", "xAxis").call(xAxis);
  svg.append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")").attr("class", "yAxis").call(yAxis);

  // 绘制图
  svg.selectAll("rect").data(dataset_1).enter().append("rect").attr("class", "myrect").on("mouseover", handleMouseOver).on("mousemove", handleMouseMove).on("mouseout", handleMouseOut).attr("x", function (d, i) {
    ...
    console.log(d);return h - yScale(d) - margin.bottom - margin.top;
...
function handleMouseOver(d) {
  ...
  // tooltip
  tooltip.html("  数据: " + d + "  ").style("left", event.pageX + "px").style("top", event.pageY + 20 + "px").style("opacity", 1.0);
}
...
var GooalColumn = function (_GooalCharts) {
    inherits(GooalColumn, _GooalCharts);

    function GooalColumn(dom, options) {
        classCallCheck(this, GooalColumn);
        return possibleConstructorReturn(this, (GooalColumn.__proto__ || Object.getPrototypeOf(GooalColumn)).call(this, dom, options));
    }

    // title


    createClass(GooalColumn, [{
        key: 'setTitle',
        ...
        key: 'drawAxis',
        value: function drawAxis(container, options) {}
        // legend
        ...
        key: 'drawLegend',
        value: function drawLegend(container, options) {}
    }, {
        key: 'draw',
